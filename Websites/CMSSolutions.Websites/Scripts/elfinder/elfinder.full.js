 ...
 * Version 2.1 (Nightly: cd569ed) (2013-03-26)
 * http://elfinder.org
 * 
 ...
 * @type String
 **/
elFinder.prototype.version = '2.1 (Nightly: cd569ed)';
...
$.fn.elfinderbutton = function(cmd) {
	return this.each(function() {
		...
		var c        = 'class',
			...
			menu,
			button   = $(this).addClass('elfinder-button')
...
$.fn.elfindercontextmenu = function(fm) {
	...
	return this.each(function() {
		var menu = $(this).addClass('ui-helper-reset ui-widget ui-corner-all elfinder-contextmenu elfinder-contextmenu-'+fm.direction)
				.hide()
				.appendTo('body')
	...
	this.not('.elfinder-cwd').each(function() {
		...
		var 
						...
						(function(node, tmb) {
							$('<img/>')
								.load(function () {
								    ...
								    node.find('.elfinder-cwd-icon').css('background-size', "48px 48px");
								})
								.attr('src', tmb);
						})(node, url+tmb);
					} else {
			...
			content = function(files, any) {
				...
				list && cwd.html('<table class="table table-bordered"><thead><tr class="ui-state-default"><td >' + msg.name + '</td><td>' + msg.perm + '</td><td>' + msg.mod + '</td><td>' + msg.size + '</td><td>' + msg.kind + '</td></tr></thead><tbody/></table>');
...
$.fn.elfinderdialog = function(opts) {
	...
	this.filter(':not(.ui-dialog-content)').each(function() {
		...
		dialog.prepend(
			$('<div class="ui-dialog-titlebar ui-corner-all ui-helper-clearfix">'+opts.title+'</div>')
		...
		$.each(opts.buttons, function(name, cb) {
			var button = $('<button type="button" class="ui-button ui-widget ui-corner-all ui-button-text-only"><span class="ui-button-text">'+name+'</span></button>')
...
$.fn.elfindernavbar = function(fm, opts) {
	...
	this.not('.elfinder-navbar').each(function() {
		var nav    = $(this).addClass('elfinder-navbar'),
			parent = nav.parent()
				.resize(function() {
...
$.fn.elfinderpanel = function(fm) {
	...
	return this.each(function() {
		var panel = $(this).addClass('elfinder-panel ui-corner-all'),
...
$.fn.elfindersortbutton = function(cmd) {
	...
	return this.each(function() {
		var fm       = cmd.fm,
			...
			asc      = selected+'-asc',
			desc     = selected+'-desc',
			button   = $(this).addClass(' elfinder-button elfinder-menubutton elfiner-button-'+name)
				.attr('title', cmd.title)
...
$.fn.elfindertoolbar = function(fm, opts) {
	this.not('.elfinder-toolbar').each(function() {
		var commands = fm._commands,
			self     = $(this).addClass('ui-helper-clearfix ui-corner-top elfinder-toolbar'),
			panels   = opts || [],
			l        = panels.length,
			i, cmd, panel, button;
...
elFinder.prototype.commands.help = function() {
	var fm   = this.fm,
		...
		tab     = '<li class=" ui-corner-top"><a href="#{id}">{title}</a></li>',
		...
		var fm    = this.fm,
			...
			open = function(file, id) {
				var dialog   = $('<div class="elfinder-dialog-resize"/>'),
					...
					uibutton    = '<div class="elfinder-button"/>',
					...
					uiprop   = $('<span />'),
					reset    = $('<div class="ui-corner-all elfinder-resize-reset"><span class="ui-icon ui-icon-arrowreturnthick-1-w"/></div>'),
...
elFinder.prototype.commands.upload = function() {
	...
	this.exec = function(data) {
		...
		button = $('<div class="ui-button ui-widget ui-corner-all ui-button-text-only"><span class="ui-button-text">'+fm.i18n('selectForUpload')+'</span></div>')
