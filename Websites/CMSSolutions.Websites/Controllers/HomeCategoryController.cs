...
namespace CMSSolutions.Websites.Controllers
{
    ...
    public class HomeCategoryController : BaseHomeController
    {
        ...
        [HttpGet]
        [Url("su-kien/phim-dang-hot.html")]
        public ActionResult FilmHot()
        {
            UrlLogin = Request.Url != null ? Request.Url.AbsoluteUri : Url.Action("Index", "Home");
            SiteId = (int)Site.Home;
            var serviceCategory = WorkContext.Resolve<ICategoryService>();
            ...
            var category = serviceCategory.GetByIdCache((int)FixCategories.FilmHot);
            ViewData[Extensions.Constants.HeaderTitle] = category.Name;
            ViewData[Extensions.Constants.HeaderDescription] = category.Description;
            ViewData[Extensions.Constants.HeaderKeywords] = category.Tags;
        ...
        private void BuildModulesCategory(CategoryInfo category)
        {
            ...
            if (category.Id == 2 || category.ParentId == 2)
            {
                var modelSlider = new DataViewerModel { Data = BuildSlider(SiteId, (int)SliderPages.CatetegoryPhimLe) };
            ...
            if (category.Id == 3 || category.ParentId == 3)
            {
                var modelSlider = new DataViewerModel { Data = BuildSlider(SiteId, (int)SliderPages.CatetegoryPhimBo) };
                var viewSlider = viewRenderer.RenderPartialView(Extensions.Constants.SliderBannerFilePath, modelSlider);
                WorkContext.Layout.SliderBanner.Add(new MvcHtmlString(viewSlider));
            }
            ...
            #endregion
            var type = 0;
            if (category.Id == (int)FixCategories.FilmLe || category.ParentId == (int)FixCategories.FilmLe)
            {
                type = 1;
            ...
            if (category.Id == (int)FixCategories.FilmBo || category.ParentId == (int)FixCategories.FilmBo)
            {
                type = 2;
            ...
            var totalRow = 0;
            var listFilmCategory = service.GetByCategoryId(
                             SiteId, WorkContext.CurrentCulture,
                             category.ParentId > 0 ? category.Id : 0,
                             category.ParentId == 0 ? category.Id : 0,
            ...
            modelFirst.Type = type;
            var viewFilmFirst = viewRenderer.RenderPartialView(Extensions.Constants.CategoryListViewViewFilePath, modelFirst);
        ...
        public override void BuildModules()
        {
            ...
            #region HomeSliderBanner
            var modelSlider = new DataViewerModel { Data = BuildSlider(SiteId, (int)SliderPages.FilmHot) };
            var viewSlider = viewRenderer.RenderPartialView(Extensions.Constants.SliderBannerFilePath, modelSlider);
            WorkContext.Layout.SliderBanner.Add(new MvcHtmlString(viewSlider));
           ...
           var data = BuilHtml(modelFirst);
            var viewFilmFirst = viewRenderer.RenderPartialView(Extensions.Constants.HomeFilmHotViewFilePath, modelFirst);
            WorkContext.Layout.CategoryContentLeftFirst.Add(new MvcHtmlString(viewFilmFirst));
            #endregion

            #region HomeFacebookFanpage
            var modelSliderOne = new DataViewCategoryModel();
            modelSliderOne.Data = data == null ? "[]" : Utilities.ConvertObjectToJson(data.Select(x => x.Id).ToList());
            var viewSliderOne = viewRenderer.RenderPartialView(Extensions.Constants.SliderOneViewFilePath, modelSliderOne);
            WorkContext.Layout.AdFacebookFanpage.Add(new MvcHtmlString(viewSliderOne));
            #endregion
        ...
        private IEnumerable<FilmInfo> BuilHtml(DataViewCategoryModel model)
        {
            var service = WorkContext.Resolve<IFilmService>();
            service.SiteId = SiteId;
            service.LanguageCode = WorkContext.CurrentCulture;
            var totalRow = 0;
            var listFilmHot = service.GetFilmHot(model.PageIndex, model.PageSize, out totalRow);
            ...
            if (listFilmHot != null && listFilmHot.Count > 0)
            {
                ...
                html.Append("<li>");
                for (int i = 0; i < listFilmHot.Count; i++)
                {
                    html.Append(BuildItem(listFilmHot[i], i, true));
                }
                html.Append("</li>");
                html.Append("</ul>");
                html.Append("</div>");
            ...
            return listFilmHot != null ? listFilmHot.ToList(): null;
        }
    }
}
